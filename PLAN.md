...
- **Rules**:
  ...
  - ✅ Block force pushes
  - Require status checks to pass — выключено до Step 0.7, после Step 0.7 добавляются `flutter analyze`, `flutter test`, `flutter build web`
...
Найденные паттерны → текущий блок (особенно пп. 1, 2, 3, 6 conventions). Test PR содержит полезный diff (добавление 0.5 и 0.6 в список закрытых шагов README) и подлежит squash-merge как первый legitimate PR в новой схеме — не «выкинут».
